...
import type { Trend } from "./useSimEngine";
import { RANGES, type Severity } from "./vitalStatus";
import type { VitalKey } from "./prototypeCase";

/** Each severity gets its own colour, ring and label — readable without a legend. */
const SEVERITY: Record<Severity, { tile: string; value: string; chip: string; text: string }> = {
  normal: {
...
export function VitalTile({
  ...
  return (
    <div
      className={cn(
        "relative flex flex-col gap-1 rounded-2xl px-3.5 py-2.5 transition-colors duration-500",
        s.tile,
    ...
    >
      ...
      <div className="flex items-baseline gap-1">
        <span className={cn("font-heading text-[26px] font-extrabold leading-none tabular-nums", s.value)}>
          {value}
        </span>
